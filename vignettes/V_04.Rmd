...
Generating a synthetic population is only part of the population-synthesis workflow. It is equally important to assess whether generated populations reproduce the distributions used during their construction.
...
Validation diagnostics can be assessed numerically using `procure()` and visually using `depict()`.
...
## Inspect validation results
Validation diagnostics are generated using `ratify()` and stored within the `ReplicaAdder`.

These diagnostics can be inspected directly using `procure()`.
...
Detailed validation results are available via `procure(ADDER, slot = "validation_results")$details`.
...
- generated validation diagnostics using `ratify()`;

- retrieved validation results using `procure()`;

- visualised validation diagnostics using `depict()`;

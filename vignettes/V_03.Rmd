...
**The functionality demonstrated in this vignette requires significant work to be fit for purposes other than this illustrative example. This vignette will be updated once this work is complete.**
...
`ReplicaStructure` objects define household-generation rules, while `ReplicaGrouper` objects coordinate household generation using `manufacture()`.
...
A `ReplicaStructure` specifies a household structure that we plan to generate. Here we create a simple couple household.

```{r}
STRUCTURE <- ReplicaStructure(household_type = "CoupleHousehold")
```

At this point we have only defined the name of the household structure. We now need to specify the household members that form this household structure.


## Define household membership
The `renew()` method is used to update `ReplicaStructure` objects.

In this example it is used to define the household-member roles that must be filled when constructing a household.
...
For simplicity, this illustrative example will generate Female - Male pairs in which the age gap between partners is within 5 years. We can use `renew` for this purpose.
...
Structures are registered using `renew()`.
...
We can retrieve the list of all structures registered with a grouper using `procure()`. In this example only one structure has been registered.
...
Once household types have been registered, the complete household-generation workflow can be executed using `manufacture()`.

```{r}
results_ls <- manufacture(GROUPER)
```

The `manufacture()` method executes the complete household-generation workflow.

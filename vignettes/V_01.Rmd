...
- `renew()` to update module contents;

- `procure()` to retrieve module contents;

- `enhance()` to assign attributes;

- `ratify()` to validate results;

- `depict()` to visualise validation outcomes; and

- `manufacture()` to generate household outputs.
...
The table contains aggregate counts rather than individual agents. We can convert this `data.frame` of aggregates to a `data.table` of individual agents:
...
The same `make_agents()` function can be used with any marginal distribution.
...
Additional attributes can be assigned using `ReplicaAdder` and the `enhance()` method. The use of this class is demonstrated in the "Assigning Attributes Using Contingency Tables" vignette.

## Assign agents to households
After additional attributes have been assigned, agents can be grouped into households.

To do this, create the `ReplicaStructure` and `ReplicaGrouper` objects and use `renew()` and `manufacture()` to define and generate household structures. These methods are described in another vignette.
...
- - introduced the broader `replica` workflow for attribute assignment, validation and household generation.

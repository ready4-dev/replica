...
The synthetic population now contains the assigned `education` attribute. We can access the updated synthetic population with the `procure` method.
...
The quality of assigned attributes can be evaluated using the validation tools provided by `replica`.
...
These tools are described in detail in the "Evaluating Synthetic Population Quality" vignette.
...
- introduced the validation workflow based on `ratify()` and `depict()`.

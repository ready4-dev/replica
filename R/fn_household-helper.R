...
#'
#' @export
createHouseholdWithId <- function(
    #createHouseholdWithId() is currently storing the entire household under the "child" slot when called from matchAdultsWithChildren(). REFACTOR
    object,
    position,
    id_offset,
    agents
...
#'
#' This function is used internally by
#' \code{\link{pairPartners}} during couple formation.
#'
#' @param object A \code{HouseholdType} object.
#'
#' @param mask Logical vector identifying agents eligible for
...
#'
#' This function is used internally by
#' \code{\link{groupChildren}} during sibling-group creation.
#'
...
matchAdultsWithChildren <- function(
  ...
  age_mother <- calculateGroupCounts(
...
pairPartners <- function(
  ...
  gender_counts <- calculateGroupCounts(
  ...
  for (pair_name in names(gender_counts)) {
    ...
    age_gap_counts <- calculateGroupCounts(
